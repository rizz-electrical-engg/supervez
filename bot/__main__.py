...
from .funcn import send_to_file_channel  # Ensure the correct relative import
from .config import OWNER, DEV  # Ensure the correct relative import

from telethon.tl.types import InputPeerChannel  # Added import
...
async def send_to_file_channel(client, file_path, caption):  # Added function
    file_channel_id = int(FILE_CHANNEL)
    file_channel = InputPeerChannel(file_channel_id, 0)
    await client.send_file(file_channel, file=file_path, caption=caption)
...
@bot.on(events.NewMessage(pattern='/sendfile'))  # Added event handler
async def handle_sendfile(event):
    if str(event.sender_id) not in OWNER and event.sender_id != DEV:
        return await event.reply("**Sorry, you're not an authorized user!**")
    
    if not event.media:
        return await event.reply("**Please attach a file to send.**")
    
    file_path = await event.client.download_media(event.media)
    await send_to_file_channel(event.client, file_path, "File sent to channel")
    await event.reply("**File sent to channel successfully.**")
    ...
    for i in itertools.count():
        try:
            if not WORKING and QUEUE:
                ...
                os.remove(dl)
                os.remove(out)
                await send_to_file_channel(e.client, out, dk)  # Added call to send_to_file_channel
            else:
